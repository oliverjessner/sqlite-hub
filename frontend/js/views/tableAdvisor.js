...
function renderFixWarnings(warnings = [], severity = 'warning') {
    ...
    const critical = String(severity).toLowerCase() === 'critical';

    return `
      <div class="mt-4 border ${critical ? 'border-error/30 bg-error-container/15 text-error' : 'border-primary-container/25 bg-primary-container/10 text-on-surface'} px-4 py-3">
        <div class="flex items-center gap-2 font-mono text-[10px] font-bold uppercase tracking-[0.18em]">
...
function renderIssue(issue = {}) {
    ...
    const fixAvailable = Boolean(issue.fixAvailable && sql);
    ...
    return `
      <article class="border border-outline-variant/10 bg-surface-container-lowest px-4 py-4">
        ...
        ${renderFixWarnings(issue.warnings ?? [], issue.severity)}
        ${
            fixAvailable
                ? `<details class="group mt-4 border border-outline-variant/10 bg-surface-container-low">
                    <summary class="flex cursor-pointer list-none items-center justify-between gap-3 px-3 py-2 font-mono text-[10px] font-bold uppercase tracking-[0.18em] text-primary-container">
                      <span class="flex items-center gap-2">
                        <span class="material-symbols-outlined text-base transition-transform group-open:rotate-90">chevron_right</span>
                        Show SQL
                      </span>
                      <span>${escapeHtml(issue.queryOrigin ?? 'Generated by Advisor')}</span>
                    </summary>
                    <pre class="custom-scrollbar max-h-64 overflow-auto whitespace-pre-wrap border-t border-outline-variant/10 px-3 py-3 text-xs leading-6 text-on-surface"><code>${highlightSql(
                        sql,
                    )}</code></pre>
                    <div class="flex flex-wrap items-center justify-end gap-2 border-t border-outline-variant/10 px-3 py-3">
                      <button
                      ...
                      <button
                        class="signature-button min-h-8 px-3 py-1 text-[10px]"
                        data-action="insert-table-advisor-sql"
                        data-issue-id="${escapeHtml(issue.id ?? '')}"
                        type="button"
                      >
                      ...
                      ${
                          issue.severity === 'critical' && issue.backupRecommended
                              ? `<button
                                  class="standard-button min-h-8 px-3 py-1 text-[10px]"
                                  data-action="backup-and-insert-table-advisor-sql"
                                  data-issue-id="${escapeHtml(issue.id ?? '')}"
                                  type="button"
                                >
                                  <span class="material-symbols-outlined text-sm">backup</span>
                                  Create backup and insert into editor
                                </button>`
                              : ''
                      }
                    </div>
                  </details>`
                : `<div class="mt-4 border border-outline-variant/15 bg-surface-container-low px-3 py-3 text-xs leading-5 text-on-surface-variant/70">
                    <span class="font-mono font-bold uppercase tracking-[0.14em] text-on-surface-variant/50">No automatic SQL fix</span>
                    <p class="mt-1">${escapeHtml(issue.fixUnavailableReason ?? 'No complete, context-safe SQL fix is available.')}</p>
                  </div>`
        }
      </article>
    `;
