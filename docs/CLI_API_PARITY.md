...
| Create stored query   | `--create-stored-query:"SQL" --title:"Name"`            | `POST /databases/:databaseId/queries`                             | Full     | Both create or update a saved definition without executing SQL.                                  |
...
| Generate schema types | `--table:name --types:typescript\|rust\|kotlin\|swift\|go` | `POST /databases/:databaseId/tables/:tableName/types`             | Partial  | Same generator. CLI supports stdout/file output and aliases; API returns JSON.                    |
