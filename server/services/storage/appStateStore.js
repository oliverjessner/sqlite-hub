...
const { resolvePathInsideDirectory, resolveUserPath } = require('../../utils/fileValidation');
const {
    buildAutoTitle,
    buildSqlPreview,
    detectQueryType,
...
function normalizeAccessLogMetadata(metadata = {}) {
    ...
    try {
        serialized = JSON.stringify(safeMetadata, (_key, value) => (typeof value === 'bigint' ? String(value) : value));
    } catch {
...
function normalizeConnectionTagName(value) {
    ...
    if (normalizedName.length > MAX_CONNECTION_TAG_NAME_LENGTH) {
        throw new ValidationError(`Tag name must be ${MAX_CONNECTION_TAG_NAME_LENGTH} characters or fewer.`);
    }
...
function isChartCompatibleQuery(queryType, rawSql = '') {
    return (
        String(queryType ?? '')
            .trim()
            .toLowerCase() === 'select' || detectQueryType(rawSql) === 'select'
    );
}
...
function normalizeLegacyDatabasePaths(legacyDatabasePaths, currentFilePath) {
    ...
    return normalizedPaths.filter((legacyPath, index, legacyPaths) => legacyPaths.indexOf(legacyPath) === index);
}
...
class AppStateStore {
    constructor(filePath, options = {}) {
        ...
        this.legacyFileUrl = this.legacyFilePath ? pathToFileURL(this.legacyFilePath) : null;
        this.legacyDatabasePaths = normalizeLegacyDatabasePaths(options.legacyDatabasePaths, this.filePath);
    ...
    migrateSettings() {
        const maxRecentConnections = this.db
            .prepare('SELECT value FROM settings WHERE key = ?')
            .get('maxRecentConnections');

        if (Number(this.parseStoredValue(maxRecentConnections?.value)) === 12) {
            this.db
                .prepare('UPDATE settings SET value = ? WHERE key = ?')
                .run(JSON.stringify(DEFAULT_STATE.settings.maxRecentConnections), 'maxRecentConnections');
        }
    }
    ...
    getExistingLegacyDatabasePaths() {
        return this.legacyDatabasePaths.filter(
            (legacyPath, index, legacyPaths) =>
                legacyPath !== this.filePath && legacyPaths.indexOf(legacyPath) === index && fs.existsSync(legacyPath),
        );
    }
    ...
    readLegacyDatabase(legacyDatabasePath) {
        ...
        try {
            ...
            const recentConnectionColumns = tables.has('recent_connections')
                ...
                : new Set();
            const logoPathSelection = recentConnectionColumns.has('logoPath') ? 'logoPath' : 'NULL AS logoPath';
            ...
            return {
                ...
                activeConnectionId: tables.has('app_meta')
                    ? (legacyDb.prepare('SELECT value FROM app_meta WHERE key = ?').get('activeConnectionId')?.value ??
                      null)
                    : null,
            };
        } finally {
    ...
    `);
        ...
        this.db.transaction(() => {
            for (const [key, value] of Object.entries(legacyState.settings ?? {})) {
                ...
                const normalizedValue = typeof value === 'string' ? this.parseStoredValue(value) : value;
            ...
            this.setMetaValue('legacyImportSource', path.relative(path.dirname(this.filePath), sourcePath));
        })();
    }
    ...
    importFirstLegacyDatabase() {
        ...
        for (const legacyDatabasePath of this.getExistingLegacyDatabasePaths()) {
            try {
                this.importStateSnapshot(this.readLegacyDatabase(legacyDatabasePath), legacyDatabasePath);
                return true;
            } catch (error) {
                console.warn(`Could not import legacy app state database from ${legacyDatabasePath}: ${error.message}`);
            }
        }
    ...
    tryImportLegacyState() {
        try {
            ...
            console.warn(`Could not import legacy app state from ${this.legacyFilePath}: ${error.message}`);
        }
    }
    ...
    buildMediaTaggingConfigRecord(row = {}) {
        return normalizeMediaTaggingConfigRecord(
            Object.fromEntries(MEDIA_TAGGING_CONFIG_FIELDS.map(field => [field.property, row[field.column] ?? null])),
        );
    }
    ...
    getMcpStatus(defaultStatus = {}) {
        ...
        return {
            ...defaultStatus,
            ...(storedStatus && typeof storedStatus === 'object' && !Array.isArray(storedStatus) ? storedStatus : {}),
        };
    }
    ...
    decorateQueryHistoryChartRow(row = {}) {
        return {
            ...
            resultColumns: this.parseQueryHistoryChartResultColumns(row.result_columns_json ?? row.resultColumnsJson),
    ...
    `;
      ...
      `,
        ...
        const queryHistoryCountSql = ['SELECT COUNT(*) AS count', baseFromSql, whereSql].join('\n');
        const countRow = this.db.prepare(queryHistoryCountSql).get(...params);
    ...
    createStoredQuery({ databaseKey, rawSql, title, notes } = {}) {
        ...
        const normalizedNotes = notes === undefined ? undefined : this.normalizeQueryHistoryText(notes);
    ...
    recordQueryExecutionInTransaction({
        ...
        `,
                ...
                .run(normalizedRawSql, queryType, serializedTables, destructive, timestamp, historyId);
        } else {
            const insertResult = this.db
                .prepare(
                    `
    ...
    listAccessLogs(options = {}) {
      ...
      `,
        ...
        const total = Number(
            this.db.prepare(`SELECT COUNT(*) AS count FROM access_log ${whereClause}`).get(...params)?.count ?? 0,
        );
    ...
    decorateQueryRunLogRow(row = {}) {
        ...
        return {
            ...
            affectedRows:
                row.affected_rows === null || row.affected_rows === undefined ? null : Number(row.affected_rows),
    ...
    listAccessActivityLogs(options = {}) {
        ...
        `,
            ...
            total: Number(
                this.db.prepare(`SELECT COUNT(*) AS count FROM access_log ${whereSql}`).get(...params)?.count ?? 0,
            ),
        };
    }
    ...
    listActivityLogs(options = {}) {
        ...
        const limit = Math.max(1, Math.min(200, Number(options.limit) || 100));
        const offset = Math.max(0, Number(options.offset) || 0);
        const queryLogs = kind === 'access' ? { items: [], total: 0 } : this.listQueryRunLogs(options);
        const accessLogs = kind === 'query' ? { items: [], total: 0 } : this.listAccessActivityLogs(options);
    ...
    updateQueryHistoryField(historyId, fieldName, value, databaseKey) {
        ...
        const statements = {
            is_favorite: this.db.prepare('UPDATE query_history SET is_favorite = ? WHERE id = ? AND database_key = ?'),
            is_saved: this.db.prepare('UPDATE query_history SET is_saved = ? WHERE id = ? AND database_key = ?'),
            title: this.db.prepare('UPDATE query_history SET title = ? WHERE id = ? AND database_key = ?'),
            notes: this.db.prepare('UPDATE query_history SET notes = ? WHERE id = ? AND database_key = ?'),
        };
    ...
    resolveUniqueQueryHistoryChartName(queryHistoryId, candidateName, { excludeChartId = null } = {}) {
        ...
        while (true) {
            ...
            const row = this.db
                ...
                .get(Number(queryHistoryId), nextName, ...(excludeChartId ? [Number(excludeChartId)] : []));
    ...
    createQueryHistoryChart({
        ...
        const requestedName = normalizeChartName(name) || buildDefaultChartName(normalizedChartType, item.displayTitle);
    ...
    updateQueryHistoryChart(
        ...
        const nextName = name === undefined ? existing.name : normalizeChartName(name);
      ...
      `,
        ...
        const nextConfig = normalizeChartConfig(nextChartType, config === undefined ? existing.config : config);
        const nextResultColumns =
            resultColumns === undefined ? existing.resultColumns : normalizeResultColumns(resultColumns);
        const nextTableVisible = tableVisible === undefined ? existing.tableVisible : Boolean(tableVisible);
        const timestamp = new Date().toISOString();
    ...
    deleteQueryHistoryChart(chartId, databaseKey = null) {
        ...
        const result = this.db.prepare('DELETE FROM query_history_chart WHERE id = ?').run(Number(chartId));
    ...
    renameQuery(historyId, title, databaseKey) {
        return this.updateQueryHistoryField(historyId, 'title', this.normalizeQueryHistoryText(title), databaseKey);
    }

    updateQueryNotes(historyId, notes, databaseKey) {
        return this.updateQueryHistoryField(historyId, 'notes', this.normalizeQueryHistoryText(notes), databaseKey);
    }
    ...
    decorateConnectionTag(row = {}) {
        return {
            ...
            ...(row.connectionCount === undefined ? {} : { connectionCount: Number(row.connectionCount ?? 0) }),
        };
    }
    ...
    getConnectionTagsByConnectionIds(connectionIds = []) {
        ...
        for (const row of rows) {
            tagsByConnectionId.get(row.connectionId)?.push(this.decorateConnectionTag(row));
        }
    ...
    getConnectionTags(connectionId) {
        ...
        return this.getConnectionTagsByConnectionIds([normalizedConnectionId]).get(normalizedConnectionId) ?? [];
    }
    ...
    getRecentConnections() {
      ...
      `,
            ...
            .all();
        const tagsByConnectionId = this.getConnectionTagsByConnectionIds(rows.map(connection => connection.id));

        return rows.map(connection => this.decorateConnection(connection, tagsByConnectionId.get(connection.id) ?? []));
    }

    getRecentConnection(id) {
        return this.getRecentConnections().find(connection => connection.id === String(id ?? '').trim()) ?? null;
    }
    ...
    assertRecentConnectionExists(id) {
        ...
        const row = this.db.prepare('SELECT id FROM recent_connections WHERE id = ?').get(normalizedConnectionId);
    ...
    setConnectionTags(connectionId, tagNames = []) {
        ...
        this.db.transaction(() => {
            this.db.prepare('DELETE FROM connection_tags WHERE connectionId = ?').run(normalizedConnectionId);
    ...
    deleteConnectionTag(tagId) {
        const result = this.db.prepare('DELETE FROM tags WHERE id = ?').run(Number(tagId));
    ...
    saveConnectionLogo(connectionId, logoUpload = {}) {
        ...
        const mimeType = String(logoUpload.mimeType ?? '')
            .trim()
            .toLowerCase();
        ...
        const logoFilePath = resolvePathInsideDirectory(this.logoDirectory, storedFileName, 'Connection logo path');
    ...
    deleteConnectionLogo(logoPath) {
        ...
        fs.rmSync(resolvePathInsideDirectory(this.logoDirectory, normalizedLogoPath, 'Connection logo path'), {
            force: true,
        });
    }
    ...
    getSettings() {
        ...
        const parsedSettings = Object.fromEntries(rows.map(row => [row.key, this.parseStoredValue(row.value)]));
    ...
    listApiTokenUsageStats(databaseKey) {
        ...
        rows.forEach(row => {
            ...
            if (startedAt && (!current.lastCallAt || Date.parse(startedAt) > Date.parse(current.lastCallAt))) {
    ...
    listApiTokens(databaseKey) {
        ...
        const useSingleTokenQueryRunFallback =
            rows.length === 1 && accessLogCallCount === 0 && apiQueryRunUsageStats.callCount > 0;
    ...
    updateDatabaseDocument(databaseKey, documentId, patch = {}) {
        ...
        const folderId = hasFolderId ? normalizeDocumentFolderId(patch.folderId) : existingDocument.folderId;
        ...
        const content = hasContent ? normalizeDocumentContent(patch.content) : existingDocument.content;
    ...
    setMediaTaggingConfig(databaseKey, config) {
        ...
        const explicitColumns = ['database_key', ...MEDIA_TAGGING_CONFIG_FIELDS.map(field => field.column)];
        const assignments = MEDIA_TAGGING_CONFIG_FIELDS.map(field => field.column + ' = excluded.' + field.column);
        ...
        this.db.prepare(upsertSql).run(...values);
